# -*- coding: utf-8 -*-
"""
Created on Sun Nov 14 22:54:07 2021
...
# this function will generate the batch data with given batch size
def generate_batch(feature_data,batch_size):
    ind = np.random.choice(feature_data.shape[0],size=batch_size)
    batch_feature = feature_data[ind]
...
def restart_KMeans(feature_data, k, iterations, restarts, batch_size):
    best_cost = []
    best_centroids = []
    for i in range(restarts): # running through the given restarts
        ...
        for j in range(iterations): # running through given iterations
            batch_feature = generate_batch(feature_data,batch_size)
            centroid_indeces = np.array(assign_centroids(batch_feature, curr_centroids))
            curr_centroids =  np.array(move_centroids(batch_feature,centroid_indeces,curr_centroids))
        cost = calculate_cost(batch_feature,centroid_indeces,curr_centroids)
...
if __name__ == '__main__':
    ...
    K = range(2,k)
    for i in K:
        best_centroids, best_cost = restart_KMeans(feature_data, i,iterations,restarts,batch_size)
